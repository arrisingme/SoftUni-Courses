...
for current_day in range(1, remaining_days + 1):
    ...
    if current_day % 2 == 0:
        total_cost += ornament_set_price * qty_of_decorations
        total_spirit += 5
    if current_day % 3 == 0:
        total_cost += (tree_skirt_price + tree_garland_price) * qty_of_decorations
        total_spirit += 3 + 10
    if current_day % 5 == 0:
        total_cost += tree_lights_price * qty_of_decorations
        total_spirit += 17
        if current_day % 3 == 0:
            total_spirit += 30
    if current_day % 10 == 0:
